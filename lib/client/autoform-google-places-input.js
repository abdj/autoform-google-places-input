  ...
  Tracker.autorun(function () {
    ...
    if (GoogleMaps.loaded()) {
      ...
      $('.af-google-place-input').geocomplete().bind("geocode:result", function(event, result) {
        ...
        var address = {};
        address[options.mappingDict['formatted_address']] = result.formatted_address ? result.formatted_address : "";
        address.geopoint = [result.geometry.location.lng(), result.geometry.location.lat()];
        ...
        _.each(result.address_components, function(component) {
          ...
          _.each(component.types, function(name){
            ...
            if( schemaName !== undefined && schemaName !== '') {
              address[schemaName] = component.short_name;
            }
        ...
        console.log(' -- address -- ');
        console.log(address);
        Session.set('abdj-google-place-input', address)
      });
    }
